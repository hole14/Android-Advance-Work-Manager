...
class MyWorker(context: Context, workerParams: WorkerParameters) : Worker(context, workerParams) {
    ...
    private fun getCurrentWeather(city: String?): Result {
        ...
        Log.d(TAG, "getCurrentWheater: $url")
        client.post(url, object : AsyncHttpResponseHandler(){
            override fun onSuccess(statusCode: Int, headers: Array<Header?>?, responseBody: ByteArray) {
                ...
                try {
                    ...
                    val currentWeather: String = responeObject.getJSONArray("weather").getJSONObject(0).getString("main")
                    val description: String = responeObject.getJSONArray("weather").getJSONObject(0).getString("description")
                    val tempInKelvin = responeObject.getJSONObject("main").getDouble("temp")
